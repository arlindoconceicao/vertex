...
This document describes the scripts created in the `lib/` folder to simulate the behavior of the Mobile Signer App and the end-to-end communication flow with the VeriFile platform.
...
## 6. M2M Integration Simulation: DIDs and Schemas Query

**Script:** `lib/simulate-mobile-api.ts`

This script demonstrates how the mobile application (or a third-party server) should perform secure queries for Decentralized Identifiers (DIDs) on the platform. To prevent unauthorized access and ensure the mobile device possesses the correct private key, the DID search requires a **Proof-of-Possession**.

### Exact Steps the Mobile App Must Reproduce:

1. **Obtaining the M2M Token:**
   - The application calculates a local HMAC SHA-256 token by combining the M2M secret key (`SIGNER_SECRET`) and the wallet owner's own DID.
   - The generated token must be sent in the `Authorization: Bearer <token>` header.

2. **Requesting the Challenge:**
   - The application makes a `POST /api/dids/search/challenge` request, sending its own ID (`x-requester-id`).
   - The API returns a random `nonce`.

3. **Generating the Ephemeral Authentication Credential (PoP):**
   - **Crucial:** The C++ library and the Mobile App do not perform an isolated signature of the *nonce* with the ML-DSA key. Instead, they issue a Verifiable Credential.
   - The mobile SDK uses the equivalent function to `walletIssueCredentialFromSchema` to create a credential whose payload contains the `nonce` generated by the API and the action `action: "did_search_auth"`.
   - This Verifiable Credential is mathematically signed by the device's wallet with the ML-DSA-65 post-quantum curve, proving identity without exposing the raw key.

4. **Target DID Search:**
   - The application converts the newly created Verifiable Credential to Base64.
   - It performs the `GET /api/dids/search?did=<target_did>` call (or by providing the CPF).
   - It provides the headers: `x-requester-id`, `x-challenge-id`, and `x-signer-auth-credential` (containing the VC in Base64).
   - The server uses `core.verifySignedCredential()` to mathematically check the signature and verify if the *nonce* in the payload matches the saved one.

5. **Receiving the DID Document and IPFS Details:**
   - Once the proof of possession is validated, the API returns the target user's DID Document containing the public keys (ML-DSA and ML-KEM).
   - The endpoint also returns the decentralized network properties: `ipfsCid` and the full access URL `ipfsUrl`.

> **Note about Schemas:** Querying schemas requires a user session (login via the Next.js interface). For convenience, the simulation script reads directly from the database to demonstrate the return structure of the Schema entity (including the new `ipfsCid` and `ipfsUrl` attributes).
