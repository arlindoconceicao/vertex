...
> **Proof-of-Possession (PoP) Auth:** The `GET /api/signer/requests/pending` endpoint requires
> cryptographic proof of possession of the signer's ML-DSA private key. The mobile app issues an in-memory
> authentication VC (containing `action: "pending_requests_auth"` and a timestamp) signed via the encrypted SQLite wallet,
> and sends it base64-encoded in the `x-signer-auth-credential` HTTP header.
> The platform verifies the credential signature against the DID's registered public key and enforces a 2-minute timestamp window (replay protection).
...
signature. Automatically filters results to match the authenticated signer's DID.

**Headers**

| Header | Type | Description |
|---|---|---|
| `x-signer-auth-credential` | `string` | Base64-encoded signed authentication Verifiable Credential generated by the wallet |
...
- `file`: O arquivo PDF criptografado (binário/envelope contendo cápsula ML-KEM, nonce, authtag e ciphertext AES-256-GCM).
- `metadata`: JSON string com os metadados brutos Proof of Existence.

```json
// Conteúdo do campo 'metadata'
{
  "requestId": "req789",
  "issuerDid": "did:ssipq:zHzcq...",
  "recipientDid": "did:ssipq:zHzcq...",
...
Polled by the Mobile Signer App to fetch encrypted credentials awaiting download by the Holder. Automatically filters results to match the authenticated signer's DID.

**Headers**

| Header | Type | Description |
|---|---|---|
| `x-signer-auth-credential` | `string` | Base64-encoded signed authentication Verifiable Credential generated by the wallet (action: `available_credentials_auth`) |
...
Called by the Mobile Signer App to download the encrypted PDF of an available credential.
Downloading the PDF triggers the automatic deletion of PII data (`vcPayload`) from the platform database to protect the recipient's privacy.

**Headers**

| Header | Type | Description |
|---|---|---|
| `authorization` | `string` | Bearer token (static M2M secret) |

**Response `200`**
- Returns the binary content (`application/octet-stream`) of the encrypted PDF envelope.
...
Simula a ação do aplicativo móvel ao receber o desafio da web. Ele gera as chaves pós-quânticas (`ML-DSA-65` e `ML-KEM-768`), assina o desafio em formato canônico, envia a requisição HTTP POST para a plataforma e **persiste o DID e as chaves privadas no banco de dados SQLite cifrado (`lib/mobile_wallet.db`)**, utilizando a chave mantida em `lib/keys.txt`:
```bash
node lib/complete-pairing.js 'PAYLOAD'
```
> **OBS:** O payload JSON deve ser copiado diretamente da plataforma web (botão *"Copy Full Payload (JSON)"* na tela `http://localhost:3000/settings`). O script salvará a senha da wallet em `lib/keys.txt` e o banco em `lib/mobile_wallet.db` para permitir simulações posteriores do aplicativo móvel.
