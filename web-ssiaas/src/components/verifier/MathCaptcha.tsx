    ...
    <div className={`p-4 rounded-xl border flex items-center justify-between gap-4 transition-colors ${isSolved ? 'bg-emerald-950/30 border-emerald-800' : 'bg-gray-800 border-gray-700'}`}>
      ...
      {!isSolved && (
        <input
          ...
          className="w-24 bg-gray-900 border border-gray-600 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow"
        />
      )}
    </div>
  );
