...
use crate::{
    ...
    wallet_core::{
        PlainPrivateKey, RowKey, StoredPrivateKey, WALLET_METADATA_ID, WalletCoreDidCreateOptions,
        WalletMetadata, decrypt_private_key, derive_row_key, encrypt_private_key,
        new_wallet_metadata, prepare_wallet_did, rekey_wallet_metadata, validate_password,
    },
};
...

    {
        ...
        tx.execute(
            ...
            params![
                ...
                &prepared.mldsa_alg,
                &prepared.mlkem_alg,
                &prepared.mldsa_public_key,
                &prepared.mlkem_public_key,
                &prepared.did_doc_json,
                prepared.did_doc_cid.as_deref(),
                &prepared.created_at
            ],
        )?;
        tx.execute(
            ...
            params![
                &prepared.did,
                &prepared.mldsa_alg,
                prepared.mldsa_private_key.encrypted_private_key.as_slice(),
                prepared.mldsa_private_key.nonce.as_slice(),
                &prepared.created_at
            ],
        )?;
        tx.execute(
            ...
            params![
                &prepared.did,
                &prepared.mlkem_alg,
                prepared.mlkem_private_key.encrypted_private_key.as_slice(),
                prepared.mlkem_private_key.nonce.as_slice(),
                &prepared.created_at
            ],
        )?;
        tx.commit()?;
...
fn map_wallet_sqlite_error(error: rusqlite::Error) -> SsiError {
    ...
    {
        SsiError::InvalidWallet("wallet password is invalid".to_string())
    } else {
...
mod tests {
    ...
    use crate::{
        ...
        encoding::multibase_base58btc_encode,
        random::random_array,
