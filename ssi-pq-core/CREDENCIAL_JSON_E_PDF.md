...
A documentação foi alinhada com o código em
`crates/ssi-pq-core/src/credential.rs`, `crates/ssi-pq-core/src/pdf.rs`,
`crates/ssi-pq-core/src/schema.rs`, `crates/ssi-pq-core/src/did.rs`,
`crates/ssi-pq-core/src/wallet.rs`,
`crates/ssi-pq-core/src/wallet_storage.rs`, `crates/ssi-pq-node/src/lib.rs`,
`crates/ssi-pq-wasm/src/lib.rs` e com os testes em `test-node/core` e
`test-wasm`.
...
O backend portável em `wallet_storage.rs`, usado pelo WASM e por adapters de
plataforma, não depende de SQLite. Ele usa uma abstração `Storage` chave-valor,
mantém o estado da wallet cifrado com AES-256-GCM, deriva a chave de estado da
senha com Argon2id e reutiliza a mesma regra de cifragem por registro para as
chaves privadas.
