...
function verifyHelpers() {
  ...
  assert.equal(b64u(bytes), item.expected.base64url);
...
function verifyAes() {
  ...
  assert.equal(b64u(plaintext), item.expected.decrypted_plaintext_base64url);
...
function verifyWalletFlow() {
  ...
  assert.equal(item.expected.mlkem.matchesEncapsulation, true);
}
...
console.log(`Verified ${manifest.vectors.length} Node vectors from test-vectors/node/manifest.json`);
