// teste-node/message/test_message_07_file_json_tamper_ciphertext_must_fail.js
//
// TESTE MESSAGE 07 — NEGATIVO: adulterar ciphertext no pacote deve falhar (offline)
//
// Executar:
//   WALLET_PASS="minha_senha_teste" RESET_WALLET=1 node teste-node/message/test_message_07_file_json_tamper_ciphertext_must_fail.js
//
// O fluxo:
...
// 5) lê o pacote cifrado, altera 1 byte do ciphertext, salva em arquivo "tampered"
...
function tamperBase64(b64) {
  ...
  if (buf.length < 8) throw new Error("ciphertext muito pequeno para tamper");
  // flip bit no meio do buffer
...
(async () => {
  ...
  const senderWalletPath = path.join(walletsDir, "msg_sender_07.db");
  const receiverWalletPath = path.join(walletsDir, "msg_receiver_07.db");
  ...
  const tamperedPkgFile = path.join(outDir, `encrypted_pkg_${ts}.tampered.json`);
  const decryptedFile = path.join(outDir, `json_decrypted_${ts}.json`);

  console.log("🚀 TESTE MESSAGE 07: NEGATIVO (tamper ciphertext must fail)");
  console.log("Config:", {
    senderWalletPath,
    receiverWalletPath,
    originalFile,
  ...
  try {
    ...
    // Tamper ciphertext
    // -------------------------
    console.log("7) Adulterando ciphertext no pacote (tamper)...");
    ...
    const originalCiphertext = pkg.ciphertext;
    const tamperedCiphertext = tamperBase64(originalCiphertext);

    if (tamperedCiphertext === originalCiphertext) {
      throw new Error("Tamper falhou: ciphertext não mudou.");
    }
    ...
    if (!failedAsExpected) {
      const leaked = fileExistsNonEmpty(decryptedFile)
        ...
        : "";
      throw new Error(`❌ BUG: decryptFile NÃO falhou com ciphertext adulterado${leaked}`);
    }
    ...
    console.log("✅ OK: decryptFile falhou com ciphertext adulterado (comportamento correto).");
    console.log("✅ OK: TESTE MESSAGE 07 passou.");
  } finally {
  ...
  console.error("❌ FALHA TESTE MESSAGE 07:", e?.message || e);
